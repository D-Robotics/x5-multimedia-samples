...
n2d_error_t create_n2d_buffer_sample()
{
	n2d_error_t error = N2D_SUCCESS;
	n2d_buffer_t src = {0};
	...
	*/
	...
	memset(output_file_name, 0, sizeof(output_file_name));
	sprintf(output_file_name, "./create_n2d_buffer_sample_hbn_%d_%d.yuv", dst.width, dst.height);
...
int main(int argc, char **argv)
{
	...
	{
		error = create_n2d_buffer_sample();
		if (N2D_IS_ERROR(error))
		{
			goto on_close;
		}
