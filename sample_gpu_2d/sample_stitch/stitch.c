...
void stitch_performance_test(struct PerformanceTestParam *param, int stitch_count)
{
	...
	for(int i = 0; i< stitch_count; i++){
		N2D_ON_ERROR(performance_test_create_buffer_with_color(param, N2D_NV12, &src[i], n2d_light_grey));
...
n2d_error_t stitch_sample()
{
	...
	N2D_ON_ERROR(n2d_util_load_buffer_from_raw_file(input_file_name, 1920, 1080, N2D_NV12, N2D_LINEAR, &src[0]));
	N2D_ON_ERROR(n2d_util_load_buffer_from_raw_file(input_file_name, 1920, 1080, N2D_NV12, N2D_LINEAR, &src[1]));
	N2D_ON_ERROR(n2d_util_load_buffer_from_raw_file(input_file_name, 1920, 1080, N2D_NV12, N2D_LINEAR, &src[2]));
	N2D_ON_ERROR(n2d_util_load_buffer_from_raw_file(input_file_name, 1920, 1080, N2D_NV12, N2D_LINEAR, &src[3]));
	...
	sprintf(output_file_name, "./stitch_sample_%d_%d.yuv", dst.width, dst.height);
