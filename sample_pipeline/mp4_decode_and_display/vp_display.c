...
static int drm_setup_kms(vp_drm_context_t *ctx)
{
	...
	for (int i = 0; i < connector->count_modes; i++)
	{
		if (connector->modes[i].hdisplay == ctx->width && connector->modes[i].vdisplay == ctx->height && !(connector->modes[i].flags & DRM_MODE_FLAG_INTERLACE))
		{
			...
			fps = __mode_vrefresh(&connector->modes[i]);
...
static void drm_init_config(vp_drm_context_t *drm_ctx, int32_t width, int32_t height, int32_t stride, int32_t h_stride)
{
	...
	drm_ctx->stride = stride;
	drm_ctx->h_stride = h_stride;
	...
	for (int i = 0; i < drm_ctx->plane_count; i++)
	{
		...
		drm_ctx->planes[i].src_stride = stride;
		drm_ctx->planes[i].src_h_stride = h_stride;
...
int32_t vp_display_init(vp_drm_context_t *drm_ctx, int32_t width, int32_t height, int32_t stride, int32_t h_stride)
{
	int32_t ret = 0;
	drmModeConnectorPtr connector = NULL;

	drm_init_config(drm_ctx, width, height, stride, h_stride);
...
int32_t vp_display_check_resolution(int32_t width, int32_t height)
{
								...
								int dma_buf_fd, int plane_index)
{
	...
	strides[0] = drm_ctx->planes[plane_index].src_stride;
	offsets[0] = 0;
	handles[1] = prime_handle.handle;
	strides[1] = drm_ctx->planes[plane_index].src_stride;
	offsets[1] = drm_ctx->planes[plane_index].src_stride * drm_ctx->planes[plane_index].src_h_stride;
	...
	{
		printf("drmModeAddFB2, w_stride:%d h_stride:%d\n", drm_ctx->planes[plane_index].src_stride, drm_ctx->planes[plane_index].src_h_stride);
		return 0;
	}
