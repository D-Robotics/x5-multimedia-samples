...
static void close_gem_handle(int drm_fd, uint32_t gem_handle)
{
	if (!gem_handle)
		return;
	struct drm_gem_close args = { .handle = gem_handle };
...
static int drm_setup_kms(vp_drm_context_t *ctx)
{
	...
	if (ctx->preferred_connector_type == DRM_MODE_CONNECTOR_DSI) {
		/* MIPI path: prefer panel preferred mode, fallback to first mode. */
	...
	if (mode->hdisplay != ctx->width || mode->vdisplay != ctx->height) {
		/*
		 * Keep source size (SRC_W/SRC_H) as frame input size, and render
		 * to panel native mode through CRTC_W/CRTC_H scaling.
		 */
		for (int i = 0; i < ctx->plane_count; i++) {
			ctx->planes[i].crtc_w = mode->hdisplay;
			ctx->planes[i].crtc_h = mode->vdisplay;
		}
	}
...
static drmModeConnector* find_connector_by_type(int fd, int preferred_type)
{
	int i = 0;

	// drmModeRes：connector，encoder，crtc，modes等。
	...
	for (i = 0; i < resources->count_connectors; i++) {
		...
		if (conn != NULL) {
			if (conn->connection == DRM_MODE_CONNECTED &&
			    conn->count_modes > 0 &&
			    (preferred_type <= 0 || conn->connector_type == preferred_type)) {
				break;
			}
...
{
	return find_connector_by_type(fd, DRM_MODE_CONNECTOR_HDMIA);
}
...
int32_t vp_display_get_max_resolution_if_not_match(int32_t width,
		int32_t height, int32_t *out_width, int32_t *out_height)
{
...
int32_t vp_display_init(vp_drm_context_t *drm_ctx, int32_t width, int32_t height)
{
	...
	int preferred_connector_type = drm_ctx->preferred_connector_type;
	...
	if (preferred_connector_type <= 0)
		preferred_connector_type = DRM_MODE_CONNECTOR_HDMIA;
	drm_ctx->preferred_connector_type = preferred_connector_type;
	...
	connector = find_connector_by_type(drm_ctx->drm_fd, drm_ctx->preferred_connector_type);
	if (connector == NULL) {
		fprintf(stderr, "No suitable connector found (type=%d)\n",
			drm_ctx->preferred_connector_type);
...
int32_t vp_display_deinit(vp_drm_context_t *drm_ctx)
{
	...
	HASH_ITER(hh, drm_ctx->buffer_map, current, tmp) {
		...
		close_gem_handle(drm_ctx->drm_fd, current->gem_handle);
		...
		unsigned int sec, unsigned int usec, void *data)
{
	...
	long interval_us = (now.tv_sec - last_flip.tv_sec) * 1000000 + (now.tv_nsec - last_flip.tv_nsec) / 1000;
	last_flip = now;
...
static void evict_old_framebuffers(vp_drm_context_t *drm_ctx)
{
	while (drm_ctx->buffer_count > drm_ctx->max_buffers) {
		dma_buf_map_t *cur = NULL, *tmp = NULL;
		dma_buf_map_t *oldest = NULL;

		HASH_ITER(hh, drm_ctx->buffer_map, cur, tmp) {
			/* Don't evict currently displayed/pending framebuffers. */
			if (cur->fb_id == drm_ctx->front_fb_id || cur->fb_id == drm_ctx->back_fb_id)
				continue;
		...
		if (oldest->fb_id)
			drmModeRmFB(drm_ctx->drm_fd, oldest->fb_id);
		close_gem_handle(drm_ctx->drm_fd, oldest->gem_handle);
		HASH_DEL(drm_ctx->buffer_map, oldest);
		free(oldest);
...
static uint32_t get_framebuffer(vp_drm_context_t *drm_ctx, int dma_buf_fd,
		int plane_index, int width, int height, int stride, int vstride)
{
	...
	if (entry) {
		...
		if (fb_info) {
			if (fb_info->width == width && fb_info->height == height &&
				...
				entry->last_used = monotonic_us();
				drmModeFreeFB(fb_info);
				return entry->fb_id;
			}
			drmModeFreeFB(fb_info);
		}
		/* size changed for the same dma fd: remove old mapping */
		if (entry->fb_id)
			drmModeRmFB(drm_ctx->drm_fd, entry->fb_id);
		close_gem_handle(drm_ctx->drm_fd, entry->gem_handle);
	...
	entry->fb_id = fb_id;
	entry->gem_handle = prime_handle.handle;
	entry->last_used = monotonic_us();
	HASH_ADD_INT(drm_ctx->buffer_map, dma_buf_fd, entry);
	drm_ctx->buffer_count++;
	evict_old_framebuffers(drm_ctx);
		...
		hb_mem_graphic_buf_t *image_frame)
{
	...
	pthread_mutex_lock(&drm_ctx->buf_mutex);
	uint32_t fb_id = get_framebuffer(drm_ctx, dma_buf_fds[0], 0, image_frame->width,
			image_frame->height, image_frame->stride, image_frame->vstride);
	if (fb_id == 0) {
	...
	/* Drain pending events first to avoid event-queue accumulation. */
	drm_process_events_once(drm_ctx);

	/* Blocking atomic commit with retry on transient ENOSPC/EBUSY. */
	int ret = -1;
	int retries = 6;
	while (retries-- > 0) {
		ret = drmModeAtomicCommit(drm_ctx->drm_fd, req, DRM_MODE_PAGE_FLIP_EVENT, drm_ctx);
		if (ret == 0)
			break;
		if (errno == ENOSPC || errno == EBUSY) {
			drm_process_events_once(drm_ctx);
			usleep(1000);
			continue;
		}
	...
	/* Wait page_flip_handler to finish state switch. */
	while (1) {
		pthread_mutex_lock(&drm_ctx->buf_mutex);
