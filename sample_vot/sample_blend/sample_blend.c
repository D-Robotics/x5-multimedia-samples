...
#define DRM_RGB_FORMAT_MAX_CAPACITY (DRM_MAX_BLEND_WIDTH * DRM_MAX_BLEND_HEIGHT * DRM_MAX_BLEND_FPS)
	...
	if(conn->connection != DRM_MODE_CONNECTED){
		...
		for (int i = 0; i < conn->count_modes; i++) {
			...
			if(((conn->modes[i].hdisplay <= DRM_MAX_BLEND_WIDTH) &&
				...
				//竖屏的情况
				((conn->modes[i].hdisplay <= DRM_MAX_BLEND_HEIGHT) &&
				(conn->modes[i].vdisplay <= DRM_MAX_BLEND_WIDTH))){
